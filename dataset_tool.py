...
def display(tfrecord_dir):
    ...
    while True:
        ...
        image = images[0].transpose(1, 2, 0)[:, :, ::-1]
        PIL.Image.fromarray(image, 'RGB').save('test-{}.png'.format(idx))
        idx += 1
        if cv2.waitKey() == 27:
            break
...
def execute_cmdline(argv):
    ...
    data_dir = '/Users/yuanxiaomao/gdrive/stylegan2/datasets/src1'
    dataset_name = 'datasets'
    dataset_obj = dataset.load_dataset(data_dir=data_dir, tfrecord_dir=dataset_name, max_label_size=0, repeat=False,
                                       shuffle_mb=0)
    print('Dataset shape =', np.int32(dataset_obj.shape).tolist())
    print('Dynamic range =', dataset_obj.dynamic_range)
    print('Label size    =', dataset_obj.label_size)
